...
def _cli_error_detail(stdout: str, stderr: str) -> str:
    ...
    ``error`` events (``message``); pi reports it on the final ``message_end``
    (``stopReason=error`` + ``errorMessage``). Surface that so ``cli agent
    exited`` is actionable instead of blank. Falls back to stderr, then a stdout
    snippet.
    """
    claude_err = ""
    codex_err = ""
...
def _codex_attach_reasoning(reply: str, reasoning: str, source: str = "codex_reasoning") -> str:
    """Fold a provider reasoning/thinking summary into the reply payload so the
    resident routes it to the collapsible disclosure instead of leaking it as a
    chat bubble. Shared by codex (source="codex_reasoning") and pi
    (source="pi_thinking").
    ...
    payload.setdefault("thinking_source", source)
...
def _log_cli_turn_timing(cmd: list[str], result: "subprocess.CompletedProcess", wall_ms: int) -> None:
    ...
    - **pi** (``--mode json``) carries per-message ``usage`` + USD ``cost`` —
      stronger than codex's cumulative estimate; we log wall_ms + those.
...
def call_agent_cli(
    ...
    _run_kwargs: dict = {"capture_output": True, "text": True, "timeout": 120}
    if _is_pi_cmd(cmd):
    ...
    if _is_pi_cmd(cmd):
        ...
        if pi_reply:
            ...
            if pi_thinking and not raw_text:
                return _codex_attach_reasoning(pi_reply, pi_thinking, source="pi_thinking")
            return pi_reply
